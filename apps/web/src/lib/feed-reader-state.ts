...
export const migrateFeedReaderState = (
  state: FeedReaderState | FeedReaderStateV1,
): FeedReaderState =>
  state.version === FEED_READER_STATE_VERSION
    ? state
    : {
        ...state,
        version: FEED_READER_STATE_VERSION,
        paginationBySource: Object.fromEntries(
          state.sources.map((source) => [source.id, createSourcePagination(source)]),
        ),
      };
